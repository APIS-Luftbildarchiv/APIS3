...
class Points2Path(object):
    ...
    def __init__(self, point_layer, display_name, close_lines=False, sort_fields=None):
        self.pointLayer = point_layer
        self.displayName = display_name
        self.closeLines = close_lines
    ...
    def make_feature(self, points):
        ...
        if len(point_list) < 2:
            raise ValueError('Can\'t make a polyline out of {0} points'.format(len(point_list)))
        if len(point_list) > 2 and self.closeLines is True:
