...
class APISImageMapping(QDockWidget, FORM_CLASS):
    ...
    def updateMappingDetails(self):
        if self.isOblique:
            ...
            if self.currentFilmInfoDict["projekt"]:
                self.uiProjectObliqueCombo.addItems(str.split(self.currentFilmInfoDict["projekt"], ";"))
            else:
                #deactivate buttons
                pass
            ...
            if self.currentFilmInfoDict["projekt"]:
                self.uiProjectVerticalCombo.addItems(str.split(self.currentFilmInfoDict["projekt"], ";"))
            else:
                #deactivate buttons
                pass
    ...
    def updatePoint(self, point, button):
        self.reloadCpLayer()
        self.imageCenterPoint = self.canvas.mapSettings().mapToLayerCoordinates(self.cpLayer, QgsPointXY(point))
        ...
        if self.cpLayer.featureCount() > 1:
            ...
            if caps & QgsVectorDataProvider.AddFeatures:
                ...
                while iter.nextFeature(ft):
                    ...
                    ctF = QgsCoordinateTransform(self.cpLayer.crs(), calcCrs, QgsProject.instance())
    ...
    def setCurrentLayout(self, film=True, mapping=False, autoimport=False, details=False):
